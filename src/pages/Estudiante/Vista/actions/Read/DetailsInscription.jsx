...
import { User, Phone, Activity, MapPinned, ArrowLeft, Heart, AlertCircle, 
    CheckCircle, XCircle, MessageSquare, Calendar, Clock, ShoppingBag, Shirt } from 'lucide-react'

import storeUniforms from "../../../../../context/store/storeUniforme"
import ModalUniforms from "../../../../Estudiante/Components/Uniforms/modalUniforms"
...
const DetailsInscripction = () => {
    ...
    // Cargar uniforme si existe
    useEffect(() => {
        const obtenerUniforme = async () => {
            if (!inscripcion._id) return
            
            try {
                const url = `${import.meta.env.VITE_BACKEND_URL}/uniforme/inscripcion/${inscripcion._id}`
                const storedUser = JSON.parse(localStorage.getItem("auth-token"))
                const headers = {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${storedUser.state.token}`
        ...
        obtenerUniforme()
    }, [inscripcion._id, recargar, modal])

    const handleUniformeRegistrado = () => {
        ...
        <>
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="grid lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            {/* Card de Uniforme Disponible - Solo si NO tiene uniforme registrado */}
                            {inscripcion?.estado === 'Aprobada' && !uniforme && (
                                <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border-2 border-emerald-200 rounded-lg p-6 shadow-sm">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="bg-white rounded-md p-4 border border-emerald-200">
                                                ...
                                                <button 
                                                    onClick={() => toggleModal("Uniforms")}
                                                    className="w-full bg-emerald-600 text-white py-2.5 px-4 rounded-md hover:bg-emerald-700 transition-colors font-medium text-sm"
                                                >
                                                    Registrar y Adquirir uniforme
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {/* Card de Uniforme Registrado - Solo si YA tiene uniforme */}
                            {uniforme && (
                                <div className="bg-white rounded-lg shadow-sm border border-emerald-200 p-6">
                                    <div className="flex items-center gap-3 mb-5 pb-4 border-b border-emerald-200">
                                        ...
                                        <span className="ml-auto px-3 py-1 text-xs font-medium rounded-full bg-emerald-100 text-emerald-700">
                                            Registrado
                                        </span>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-900 font-medium">{uniforme.nombre || 'N/A'}</p>
                                            </div>
                                            
                                            <div>
                                                <p className="text-xs text-gray-500 mb-1">Talla</p>
                                                <p className="text-sm text-gray-900 font-medium">{uniforme.talla || 'N/A'}</p>
                                            </div>
                                        </div>

                                        {uniforme.detalle && (
                                            <div className="bg-emerald-50 p-4 rounded-md border border-emerald-100">
                                                <p className="text-xs text-gray-600 font-medium mb-2">Detalles / Observaciones</p>
                                                <p className="text-sm text-gray-700">{uniforme.detalle}</p>
                                            </div>
                                        )}

                                        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 p-4 rounded-md border border-emerald-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-xs text-gray-600 mb-1">Precio del uniforme</p>
                                            ...
                                            {uniforme.fechaRegistro && (
                                                <p className="text-xs text-gray-600 mt-3">
                                                    Registrado el {new Date(uniforme.fechaRegistro).toLocaleDateString('es-ES', {
                                                        year: 'numeric',
                                                        month: 'long',
                                                        day: 'numeric'
                                                    })}
            ...
            {modal === "Uniforms" && (
                <ModalUniforms 
                    ...
                    onRegistroExitoso={handleUniformeRegistrado}
                />
            )}
        </>
    )
